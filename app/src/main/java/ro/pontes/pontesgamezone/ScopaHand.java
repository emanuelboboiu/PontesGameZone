...
public class ScopaHand extends Hand {
	...
	public ArrayList<Card> decideCardsToBeExtracted(Card c) {
		 ...
		 * table:
		 */
		ArrayList<Card> toTakeList = new ArrayList<Card>();
		 ...
		 */
		if (isContinue
				&& (ScopaActivity.lsVariant != 2 && ScopaActivity.lsVariant != 4)) {
			/*
		...
		if (isContinue) {
			 ...
			 */
			ArrayList<ArrayList<Card>> combinations = getSumsOfCards(c
					.getValue());
	...
	public int aiBestMove(ScopaHand curHand) {
		...
		if (curHand.getCardCount() > 1) {
			 ...
			 * in hand:
			 */
			ArrayList<ArrayList<Card>> bestCombinations = new ArrayList<ArrayList<Card>>();
			...
			for (int i = 0; i < curHand.getCardCount(); i++) {
				...
				if (combinations.size() > 0) {
					...
					ArrayList<Card> falseCombination = new ArrayList<Card>();
