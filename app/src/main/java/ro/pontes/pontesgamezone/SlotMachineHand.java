...
public class SlotMachineHand extends DigitsHand {
	...
	private static int[] aFaces = new int[] { 0, 1, 2, 3, 5, 7 };

	// A constructor:
	public SlotMachineHand(Context context) {
		super(context);
		super.aPossesion[1] = context.getString(R.string.sm_cylinder_face);
		super.resPossesionString = context
				.getString(R.string.sm_cylinder_face_extended);
	} // end constructor.
	...
	public int getWinMultiplier() {
		...
		if (getDigitCount() == 3) {
			...
			if (isContinueChecking) {
				...
				isContinueChecking = false; // just for fun.
	...
	public void drawRollingCylinders(LinearLayout ll) {
		...
		ArrayList<Digit> tempHand = new ArrayList<Digit>();
		...
		for (int i = 0; i < 3; i++) {
			...
			String tempString = String.format(resPossesionString,
					aPossesion[1], tempDigit.toString());
	...
	public static int getRandomFace() {
		...
		int curFace = aFaces[GUITools.random(0, aFaces.length - 1)];
		return curFace;
	} // end getRandomFace().
